module IntegerSet {
    ...
    class Set {
        ...
        function intersect_length(s1 : seq<int>, s2 : seq<int>, count : int, i : int, stop : int) : int 
        requires 0 <= i <= stop
        requires stop <= |s1|
        decreases stop - i
        {
            if i == stop then count else (if s1[i] in s2 then intersect_length(s1, s2, count + 1, i + 1, stop) else intersect_length(s1, s2, count, i + 1, stop))
        }
        ...
        // ensures |intersection.elements| == intersect_length(this.elements, s.elements, 0, 0, |this.elements|)
        ensures fresh(intersection)
        {
            ...
            invariant 0 <= |inter| <= i <= |this.elements|
            {
        ...
        // ensures |union.elements| == union_length(this.elements, s.elements, |s.elements|, 0, |this.elements|)
        ensures fresh(union)
        {
